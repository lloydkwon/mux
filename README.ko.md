...
- **상시 패널** — `mux watch`가 창 왼쪽 전용 pane에 세션 목록과 상태 변화 로그를 띄우고, 고른 세션의 실시간 출력을 그 옆에 표시. 클릭하면 읽고 한 번 더 클릭하면 이동
...
목록은 각 세션이 *지금* 무엇을 하는지는 보여주지만 방금 무엇을 끝냈는지는 남기지 않습니다. `mux watch`는 창 **왼쪽**의 전용 pane을 세션 목록, 최근 상태 변화 로그, 그리고 고른 세션의 실시간 출력으로 채워, TUI를 열었을 때만이 아니라 작업하는 내내 보이게 합니다:
...
**클릭하면 읽고, 한 번 더 클릭하면 이동합니다.** 첫 클릭은 오른쪽 칸이 그 세션을 가리키게 할 뿐인데, 그게 핵심입니다 — 다른 세션이 무엇을 묻는지 확인하는 데 지금 타이핑하던 pane을 내놓을 이유가 없습니다. 나눌 수 없을 만큼 좁은 pane(76칸 미만)은 목록만 보여주고, 거기서는 읽을 것이 없으니 한 번의 클릭이 곧 이동입니다.

패널이 들어 있는 그 세션은 `◀`로 표시되고, 패널이 대신 골라 줄 때는 **건너뜁니다**. 그 세션의 화면은 패널 바로 옆 pane에 이미 떠 있고, 게다가 거의 항상 목록 첫 줄입니다 — 목록은 상태가 가장 최근에 바뀐 순인데, 상태가 계속 바뀌는 건 지금 작업 중인 세션이니까요. 그대로 두면 패널을 열자마자 옆 pane의 사본이 뜹니다. 직접 고르는 것은 여전히 되고, 그때는 미러 대신 **요약**이 나옵니다: 위치, 창 수, 가동 시간, 그리고 지금까지의 비용.

키보드로 쓰려면 `mux nav`를 바인딩하세요. `send-keys`로 패널에 닿기 때문에 **포커스는 내 pane에 그대로 두고** 선택만 움직입니다:
...
패널 폭은 pane 테두리를 드래그해 조절합니다 — 테두리 드래그는 tmux가 직접 처리하고 pane으로 넘기지 않으므로 클릭 기능과 충돌하지 않습니다. 조절한 폭은 **세션별로 기억**되어(tmux user option에 저장, 세션과 수명이 같음) 다시 열 때 그 폭으로 나옵니다. 처음 여는 패널은 여유가 있는 창에서는 두 칸이 다 들어가는 84칸으로, 그렇지 않은 창에서는 48칸으로 열립니다.
