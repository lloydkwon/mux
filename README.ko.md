...
**커서가 놓인 세션은 자기 이력을 펼칩니다.** 예전에는 목록 아래에 시간순 로그 하나가 깔렸는데, 실제 서버에서는 그게 제일 바쁜 세션 하나가 같은 말을 반복하는 마흔 줄이었습니다 — *이* 세션에 무슨 일이 있었는지 보려면 그걸 다 지나쳐야 했습니다. 이제 각 세션의 전환은 자기 행 아래에 붙고, 커서가 놓인 하나만 펼쳐집니다. 새로 누를 것은 없습니다: `M-Up`/`M-Down`이 이미 커서를 옮기고, 클릭과 `enter`는 여전히 "이 세션으로 이동"입니다. 이력은 세션 목록이 쓰고 남은 줄만, 그것도 상한을 두고 씁니다 — pane은 아래에서 말없이 잘리고, 남아야 할 것은 세션 쪽이기 때문입니다.
