...
- **Claude 진행 상태 표시** — 각 세션의 Claude가 작업 중인지(`⏳`), 사용자
  입력을 기다리며 막혀 있는지(`❗`), 턴이 끝나 입력을 기다리는지(`✅`)를
  경과 시간과 함께 표시합니다. 프리뷰에는 무엇 때문에 막혔는지(권한 승인
  프롬프트, 입력 필요 등)도 함께 보여줍니다.
...
- **Claude 진행 상태** — 작업 중(`⏳`) / 승인 대기(`❗`) / 입력 대기(`✅`)를 경과 시간과 함께 표시. 화면 출력을 추측하지 않고 Claude Code의 상태 파일을 직접 읽음
