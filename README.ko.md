...
**알림 줄은 작업을 부르거나, 재어서 말합니다.** 배지가 이미 "하는 중"인지 "끝났는지"를 말하므로 글자는 그것이 못 하는 걸 맡습니다 — Claude가 그 턴에 붙인 이름(`⏳ panel-session-last-notification`)입니다. 단, Claude가 스스로 붙인 이름일 때만입니다. 사용자가 손으로 바꾼 이름은 계속 그대로 남아 있어서, 전환마다 같은 단어를 반복하는 꼴이 됩니다 — 작업 이름을 붙인 이유가 바로 그걸 없애려던 것이었습니다. 그런 세션은 대신 걸린 시간을 보여줍니다(`✅ 작업 완료 · 3m 12s`). 둘 다 없는 도구는 원래 라벨 그대로입니다.
...
**왼쪽 클릭은 전환, 오른쪽 클릭은 에디터.** 왼쪽 클릭은 지금까지처럼 tmux 클라이언트를 그 세션으로 옮깁니다. 오른쪽 클릭은 그 세션의 프로젝트 폴더를 VS Code 로 열고 **클라이언트는 옮기지 않습니다** — 지금 일하던 터미널을 떠나지 않고 다른 프로젝트의 에디터만 여는 것이 이 제스처가 있는 이유이고, 옮기고 싶으면 왼쪽 버튼이 바로 옆에 있습니다. 켜야 할 옵션은 없습니다: 제스처 자체가 opt-in 입니다. 목록을 훑어보다가 오른쪽 버튼을 누르는 사람은 없으니까요.

여는 폴더는 세션에 `@project_dir` 태그가 있으면 그것, 없으면 현재 디렉터리입니다 — `@project_dir` 은 세션이 열린 폴더로 고정이지만 디렉터리는 활성 pane 을 따라 움직이고 AI 의 작업 디렉터리로 덮이기까지 하기 때문입니다. `code` 는 `PATH` 를 먼저 보고, 없으면 표준 설치 위치(deb/rpm, snap, flatpak, `/opt`, macOS 앱 번들)를 찾습니다. WSL 에서는 Windows 설치본이 interop 으로 `PATH` 에 올라와 있어 별도 설정 없이 Remote-WSL 창으로 열립니다. `code` 를 찾지 못하면 오른쪽 클릭은 아무 일도 하지 않습니다. 실행이 실패하면 삼키지 않고 패널 로그에 남깁니다 — WSL 세션보다 오래 사는 tmux 서버는 죽은 `WSL_INTEROP` 을 물려주고, 그 뒤로는 모든 실행이 실패하는데 그게 보고되지 않으면 클릭이 아무 일도 안 하는 것처럼 보이기 때문입니다.

tmux.conf 에 더할 것은 없습니다. tmux 의 기본 `MouseDown3Pane` 바인딩은 앱이 마우스 리포팅을 켜 둔 pane 에는 오른쪽 클릭을 그대로 넘겨주는데, 패널이 바로 그런 pane 입니다 — 가정이 아니라 실제 서버에서 확인했습니다. 키보드는 뜻이 하나입니다: `enter` 와 `prefix + Enter` 는 왼쪽 클릭과 같이 전환만 합니다.
...
패널 폭은 pane 테두리를 드래그해 조절합니다 — 테두리 드래그는 tmux가 직접 처리하고 pane으로 넘기지 않으므로 클릭 기능과 충돌하지 않습니다. 드래그한 폭은 디스크에 기억되고(`~/.config/mux/panel.json`) 열려 있는 모든 패널이 다음 tick에 그 폭을 따라갑니다 — 마우스도 하나, 숫자도 하나이니까요. 처음 여는 패널은 36칸입니다.

행은 **화면에 찍히는 값 순서**, 곧 그 상태를 얼마나 오래 쥐고 있었는지 순으로 최근이 위에 옵니다. 다른 기준으로 정렬하면 그 칸이 단조롭지 않게 되는데, 41m짜리가 3h짜리 둘 밑에 있으면 고장으로 읽힙니다. 상태가 바뀌면 행은 실제로 움직이고, 그래서 세션 블록에는 알림 줄과 그 아래 빈 줄이 함께 포함됩니다 — 클릭 표적은 한 행이 아니라 블록 전체입니다.
...
| `r` | 세션 이름 변경 |
| `x` | 세션 삭제 (확인 후) |
| `m` | 선택한 세션의 메모 편집 (비우면 삭제) |
| `v` | 선택 모드 — 마우스를 터미널에 돌려줘 드래그 복사 |
...
**세션에는 직접 쓴 메모를 붙일 수 있다**(`m`). 배지 다음에 `✎ …`로 그려진다. 들여다봐서는 알 수 없는 것 — "라벨링이 끝나야 진행된다" 같은 것 — 을 적어두는 자리라, 세션이 일곱 개가 되어도 각각이 무엇을 기다리는지 목록만 보고 알 수 있다. 메모는 tmux 세션 자체(`@mux_note` 옵션)에 붙으므로 이름을 바꿔도 따라오고 `mux watch`도 별도 저장소 없이 같은 값을 본다. 대신 tmux 서버가 죽으면 같이 사라진다. 행에 자리가 모자라면 **브랜치가 먼저** 사라지고 메모가 나중이다 — 브랜치는 프리뷰 헤더와 pane 위 border 가 다시 말해주지만 메모는 다른 어디에도 없기 때문이다. 패널에서는 세션 아래 별도 줄로 그려진다. 36칸짜리 행에는 남는 칸이 없다.

`v`는 마우스를 터미널에 돌려줘 화면의 글자를 긁어 복사할 수 있게 하고, **그동안 갱신을 멈춘다** — 0.5초마다 다시 그리는 목록 위에서는 선택을 끝내기도 전에 지워진다. `v` 또는 `Esc`로 둘 다 복귀한다.
