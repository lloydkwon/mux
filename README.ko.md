...
**모든 세션이 마지막에 무슨 일이 있었는지 한 줄로 말하고, 커서가 놓인 세션만 이력을 펼칩니다.** 예전에는 목록 아래에 시간순 로그 하나가 깔렸는데, 실제 서버에서는 그게 제일 바쁜 세션 하나가 같은 말을 반복하는 마흔 줄이었습니다 — *이* 세션에 무슨 일이 있었는지 보려면 그걸 다 지나쳐야 했습니다. 이제 각 세션이 마지막에 무슨 일이 있었는지 한 줄로 달고, 시간순 로그는 목록 아래에 따로 놓여 전체를 한눈에 읽습니다. 공유 로그는 50건 상한에 밀려 사라질 세션의 최신 한 건을 남기므로, 바쁜 세션이 조용한 세션을 목록에서 통째로 지워버리지 않습니다. 새로 누를 것은 없습니다: `M-Up`/`M-Down`이 이미 커서를 옮기고, 클릭과 `enter`는 여전히 "이 세션으로 이동"입니다. 이력은 세션 목록이 쓰고 남은 줄만, 그것도 상한을 두고 씁니다 — pane은 아래에서 말없이 잘리고, 남아야 할 것은 세션 쪽이기 때문입니다.
