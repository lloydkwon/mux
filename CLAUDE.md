...
Only the selected session is captured, only when there is a column to draw it in, and a `previewLoadedMsg` whose key no longer matches the selection is dropped — the same guard `viewMain` uses. `tokenUsageLoadedMsg` rides the same rule.

**The panel never previews its own session.** `ownSession` is resolved once in `RunWatch` (`tmux.SessionForPane(selfPane())`) and does two things: `autoSelect` looks past it, and `showsSelfCard` makes the detail column draw `detailSelfCard` instead of a capture. Without the first, the column opens mirroring the pane beside it — not occasionally, but almost always, because rows are ordered by how recently a state changed and the session you are working in is the one whose state keeps changing. Without the second, a lone session still mirrors. Two rules on top: skipping applies to *auto*-selection only, so a deliberate click on your own session stands; and an empty `ownSession` means "could not tell" and restores the pre-`ownSession` behavior rather than guessing either way. `showsSelfCard` is one decider read by both the renderer and the loader — two copies would drift into capturing output nothing draws.
