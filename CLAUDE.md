...
There is **a budget, because there was never one before**: `notifyLines` renders the session block twice, once with no history to measure it, then again with `expandBudget(height − header − measured)`. `fixedBox` clips from the bottom silently, so an unrationed history pushes sessions off the pane — and the sessions are the panel. `maxExpandedEvents` caps it besides, since room left over is not a reason to fill it. Computing the block's height arithmetically instead would put its layout rules in two places; rendering twice costs string assembly, and `sessionLines()` already redraws everything on every click.

The history is drawn **unhighlighted** while the session row and its reason line are not, and the block's trailing blank drops its highlight when a history is open. One extra inverted line reads as part of the row; a dozen reads as the whole block having moved, and an inverted bar under plain rows reads as a stray row rather than as the bottom of the block.
