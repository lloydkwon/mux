...
**Every session shows what last happened to it, and the one under the cursor opens its full history.** It used to be one flat chronological log below the list, and on a real server that was forty rows of whichever session was busiest repeating itself — finding what happened in *this* session meant reading past all of it. Each session now carries one line saying what last happened to it, and the chronological log sits below the list where it can be read as a whole. The shared log keeps the newest entry of every session its fifty-entry cut would otherwise silence, so a busy session cannot evict a quiet one out of the list entirely. Nothing new to press: `M-Up`/`M-Down` already move the cursor, and click and `enter` are still "switch to this session". The history takes only the rows the session list left over, capped, because the pane clips from the bottom without saying so and the sessions are what must survive.

**A logged line names the work, or times it.** The glyph already says working or done, so the words carry what it cannot — Claude's own name for the turn (`⏳ panel-session-last-notification`). That is used only where Claude chose the name itself; a session you renamed by hand keeps that label indefinitely, and repeating a fixed word on every transition is exactly what naming the work was meant to stop. Those get the turn's duration instead (`✅ 작업 완료 · 3m 12s`). Tools that publish neither keep the plain label.
...
**Left click switches, right click opens the editor.** A left click moves the tmux client to that session, as it always has. A right click opens that session's project folder in VS Code and *does not* move the client — opening a project's editor without leaving the terminal you are working in is the case it exists for, and the left button is right there for the other one. There is no option to turn on: the gesture is the opt-in, because a right click is not something anyone presses while skimming a list.

The folder is the session's `@project_dir` when it has one, and its current directory otherwise — `@project_dir` is fixed to the folder the session was opened for, while the directory follows the active pane and gets overwritten by an AI's own working directory. mux looks for `code` on `PATH` first and then in the usual install locations (deb/rpm, snap, flatpak, `/opt`, macOS app bundles); under WSL the Windows install arrives on `PATH` over interop and opens a Remote-WSL window with no extra configuration. If no `code` is found the right click does nothing. A launch that fails is reported in the panel's own log rather than swallowed — under WSL a tmux server that outlives its WSL session carries a dead `WSL_INTEROP`, and every launch after that fails in a way that would otherwise look like the click doing nothing at all.

Nothing has to be added to your tmux.conf. tmux's own `MouseDown3Pane` binding forwards a right click to a pane whose application has mouse reporting on, which the panel does — verified against a real server rather than assumed. The keyboard keeps its one meaning: `enter` and `prefix + Enter` switch, the same as a left click.
